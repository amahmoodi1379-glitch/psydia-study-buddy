...
export async function handleStats(request: Request, env: Env, pathname: string, origin: string) {
  ...
  if (pathname === "/api/app/v1/stats/heatmap") {
    const subtopics = await sbSelect(env, "subtopics", "is_active=eq.true&order=display_order.asc", "id,title_fa,topic_id,display_order");
    const subtopicIds = subtopics.map((sub: any) => sub.id);
    if (!subtopicIds.length) return json({ subtopics: [] }, 200, origin);

    const questions = await sbSelect(env, "questions", `subtopic_id=in.(${subtopicIds.join(",")})&is_active=eq.true`, "id,subtopic_id");
    ...
    const statesBySubtopic = new Map<string, any[]>();
    ...
    const subtopicHeatmap = subtopics.map((sub: any) => {
      ...
      const confidencePercent = confidenceScores.length
